...
export function aggregateSnapshots(snapshots: MachineSnapshot[]) {
  ...
  return {
    ...
    oeePct: avg((x) => x.oeePct ?? 0),

    availabilityPct: avg((x) => x.availabilityPct ?? 0),

    performancePct: avg((x) => x.performancePct ?? 0),

    qualityPct: avg((x) => x.qualityPct ?? 0),

    goodCount: sum((x) => x.goodCount ?? 0),

    rejectCount: sum((x) => x.rejectCount ?? 0),
    ...
    actualCycleTimeSec: avg((x) => x.actualCycleTimeSec ?? 0),

    idealCycleTimeSec: avg((x) => x.idealCycleTimeSec ?? 0),

    actualRatePph: avg((x) => x.actualRatePph ?? 0),

    idealRatePph: avg((x) => x.idealRatePph ?? 0),
    ...
    speed: avg((x) => x.actualRatePph ?? x.speed ?? 0),
    ...
    oeeGapPct: avg((x) => x.oeePct ?? 0) - avg((x) => x.targetOeePct ?? 85),
    availabilityGapPct: avg((x) => x.availabilityPct ?? 0) - avg((x) => x.targetAvailabilityPct ?? 90),
    performanceGapPct: avg((x) => x.performancePct ?? 0) - avg((x) => x.targetPerformancePct ?? 95),
    qualityGapPct: avg((x) => x.qualityPct ?? 0) - avg((x) => x.targetQualityPct ?? 99),
