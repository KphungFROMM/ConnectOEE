import type { LineNode, NodeKpi, PlantNode } from '../../lib/hierarchy'
import type { MachineSnapshot } from '../../lib/liveHub'
import {
  resolveLineTopology,
  rollUpContinuousLive,
  rollUpContinuousNodeKpis,
} from '../../lib/lineTopology'
...
function findLineNode(tree: PlantNode[], lineId: string): LineNode | null {
...
/** Sum/average live reliability fields across all machines on a line (Continuous: rates from output). */
export function rollupLineLiveMetrics(
  snapshots: MachineSnapshot[],
  lineId: string,
  line?: LineNode | null,
): ExplorerLiveMetrics | null {
  const snaps = snapshots.filter((s) => s.lineId === lineId)
  const base = rollupSnapshots(snaps)
  if (!base || !line || (line.topology ?? 'Independent') !== 'Continuous' || snaps.length <= 1) return base
  const resolved = resolveLineTopology(
    ...
    snaps.map((s) => s.machineId),
  )
  if (!resolved.outputId || !resolved.pacingId) return base
  const c = rollUpContinuousLive(snaps, resolved.outputId, resolved.pacingId)
  return {
    ...base,
...
export function liveMetricsForExplorerNode(
  ...
  if (node.level === 'Line' && node.lineId) {
    const line = tree ? findLineNode(tree, node.lineId) : null
    return rollupLineLiveMetrics(snapshots, node.lineId, line)
  }
...
export function teepPctForExplorerNode(
  ...
  if (node.level === 'Line') {
    ...
    if (tree && lineId) {
      ...
      if (line && (line.topology ?? 'Independent') === 'Continuous' && snaps.length > 0) {
        const resolved = resolveLineTopology(
          {
            topology: 'Continuous',
            lineOutputMachineId: line.lineOutputMachineId,
        ...
        const out = snaps.find((s) => s.machineId === resolved.outputId) ?? snaps[snaps.length - 1]
        return out?.teepPct ?? 0
      }
    }
...
function rollUpLineFromMachines(line: LineNode, machines: { id: string; kpi: NodeKpi }[]): NodeKpi {
  if ((line.topology ?? 'Independent') !== 'Continuous' || machines.length <= 1) {
    return rollUpKpis(machines.map((m) => m.kpi))
  }
  const resolved = resolveLineTopology(
    {
      topology: 'Continuous',
  ...
  if (!resolved.outputId || !resolved.pacingId) return rollUpKpis(machines.map((m) => m.kpi))
  return rollUpContinuousNodeKpis(machines, resolved.outputId, resolved.pacingId)
}
  ...
  return tree.map((plant) => {
    const departments = plant.departments.map((dept) => {
      const lines = dept.lines.map((line) => {
        ...
        return { ...line, machines, kpi: rollUpLineFromMachines(line, machines) }
      })
