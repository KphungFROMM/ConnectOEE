...
- **Phase 11 - Rockwell driver (libplctag):** real ControlLogix/CompactLogix read/write (start permissive, reset/ack), UDT enumeration, smart fault-code mapping, multi-PLC per line (see 08). (Step 6 real + fault mapping) **[Implemented]** `RockwellDriver` (libplctag) reads mapped signals, writes narrow control commands, and browses controller tags + UDTs; `DriverFactory`/`DriverRegistry` run multiple drivers (Mock + Rockwell) per the configured connections with a Mock fallback; fault attribution uses DB `FaultCodeMap` then the `RockwellFaultCatalog` default banding. **Live read/write against physical ControlLogix/CompactLogix hardware is the only deferred item** (no controller on hand yet) - the code path is complete and degrades to Faulted/empty when no PLC is reachable.
- **Phase 12 - Admin screens & packaging:** remaining admin config UIs, Windows Service packaging + installer, single-port local serving, retention/backup ops, final hardening. (Step 16) **[Implemented]** Admin **System** tab surfaces live driver/connection health, version/uptime/DB state, the TimescaleDB retention/compression policy table, and on-demand `pg_dump` backups. Single-port serving (SPA + API + SignalR) and `UseWindowsService` are wired in `Program.cs`; `scripts/build-release.ps1` stages the SPA into `wwwroot` and publishes the API, and `scripts/install-service.ps1` / `uninstall-service.ps1` register the Windows Service.
