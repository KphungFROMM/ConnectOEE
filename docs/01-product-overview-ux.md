...
- **Plant Explorer** is the live hierarchical cockpit with a **single sticky command plane** (breadcrumb, search, filters, scope identity, current shift — plant/dept via plant calendar, line/machine via line calendar, never gated on PLC live), then fleet/APQ health strip, explorer-native child tiles, **one** KPI hero, and URL-synced Overview / Downtime / Reliability tabs. Default view is shift-to-date with optional last-8h on the Overview trend. Active shift is never shown in the global app header (multi-plant calendars differ); Operator Station and Analytics show shift for their selected plant/line scope.
- **All plants** shows fleet health (running/down/offline + lowest-OEE shortcuts) and plant tiles with shift chips; a single-plant site auto-enters that plant.
